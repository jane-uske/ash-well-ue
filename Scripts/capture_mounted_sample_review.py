...
p=argparse.ArgumentParser();p.add_argument('mode',choices=['asset','charge','death']);p.add_argument('--output-directory',type=Path);p.add_argument('--foot-placement',action='store_true');p.add_argument('--continuous-submixes',action='store_true',help='UE-native silence rendering for recording clock verification.');p.add_argument('--width',type=int,default=1920);p.add_argument('--height',type=int,default=1080);a=p.parse_args()
...
flags=['-MountedAssetReview','-MountedReviewOrbit'] if a.mode!='charge' else ['-MountedChargeSample']
cmd=[engine,str(R/'AshWell.uproject'),'/Game/AshWell/MountedBoss/L_MountedCourtyard','-game','-windowed',f'-ResX={a.width}',f'-ResY={a.height}','-NoSplash','-CombatPrototype','-MountedExperiment','-MountedDebug','-DisablePlugins=AllToolsets,ModelContextProtocol',f'-MountedReviewRecord={a.mode}',f'-abslog={out}/current-{a.mode}-engine.log',*flags]
...
version=snapshot();(out/'version.json').write_text(json.dumps(version,indent=2)+'\n')
