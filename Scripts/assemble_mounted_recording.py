...
from audit_mounted_recording import audit_raw,probe
...
audit=audit_raw(d)
video_errors=[x for x in audit['errors'] if x not in ('missing_or_invalid_wav','truncated_wav','audio_wall_duration_mismatch')]
if video_errors or (audit['errors'] and not a.video_only_preview):
    raise SystemExit('Recording rejected by timeline audit: '+', '.join(audit['errors']))
...
for i,f in enumerate(frames):
    ...
    lines+=['file '+"'"+f['file']+"'",'option framerate 1000',f'duration {max(.001,end-start):.6f}']
lines+=['file '+"'"+frames[-1]['file']+"'",'option framerate 1000']
...
cmd+=['-c:v','libx264','-preset','fast','-crf','20','-pix_fmt','yuv420p','-fps_mode','vfr','-enc_time_base','1:1000','-t',str(meta['duration_seconds']),'-movflags','+faststart']
...
report['timeline_audit']=audit;report['encoded_media']=probe(output)
