...
with bpy.data.libraries.load(str(R/'SourceAssets/WardenHQ/WardenHQ_Prepared.blend'),link=False) as (src,dst):dst.objects=[n for n in src.objects if n.startswith('SM_WardenHQ_') and 'Hammer' not in n]
parts={}
for o in dst.objects:s.collection.objects.link(o);parts[o.name.removeprefix('SM_WardenHQ_').split('.')[0]]=o
...
for side,sign in [('Right',-1),('Left',1)]:
 ...
 parts[side+'Thigh'].matrix_world=M@link(A,B,A,K);parts[side+'Shin'].matrix_world=M@link(B,C,K,E);parts[side+'Foot'].matrix_world=M@Matrix.Translation(E-C)
...
for n,o in parts.items():
 if not any(x in n for x in ['Thigh','Shin','Foot']):o.matrix_world=M
...
s.render.engine='BLENDER_EEVEE';s.render.resolution_x=1200;s.render.resolution_y=1000;s.render.resolution_percentage=100;s.view_settings.view_transform='AgX';s.render.image_settings.file_format='PNG';s.render.filepath=str(O/'seated-legs-diagnostic.png');bpy.ops.render.render(write_still=True)
(O/'seated-legs-verification.json').write_text(json.dumps(report,indent=2));print('SEAT_CHECK',json.dumps(report))
