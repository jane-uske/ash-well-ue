...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('mode',choices=['asset','charge','death','size','moves']);a=p.parse_args()
require_unlocked()
...
delay=15 if a.mode=='charge' else 3
seconds=52 if a.mode=='moves' else 30
cmd=[str(R/'Scripts/launch_mounted_charge_sample.command'),'-MountedReviewExternal',f'-MountedReviewRecord={a.mode}',f'-MountedReviewStartDelay={delay}',f'-MountedReviewSeconds={58 if a.mode=="moves" else 45}',f'-abslog={O}/engine.log']
if a.mode in ['asset','death','size']:cmd+=['-MountedAssetReview','-MountedReviewOrbit']
with (O/'console.log').open('w') as f:
    ...
    try:
        ...
        with (O/'capture.log').open('w') as capture_log:
            result=subprocess.run([str(binary),str(proc.pid),str(O/'window-uncut.mp4'),str(seconds)],stdout=capture_log,stderr=subprocess.STDOUT,timeout=seconds+25)
